
# Write your code here!
def game_hash
  game_hash =
  {
    home:
    {
      team_name: "Brooklyn Nets",
      colors: ["Black", "White"],
      players:
      {
        "Alan Anderson" =>
        {
          number: 0,
          shoe: 16,
          points: 22,
          rebounds: 12,
          assists: 12,
          steals: 3,
          blocks: 1,
          slam_dunks: 1
        },
        "Reggie Evans" =>
        {
          number: 30,
          shoe: 14,
          points: 12,
          rebounds: 12,
          assists: 12,
          steals: 12,
          blocks: 12,
          slam_dunks: 7
        },
        "Brook Lopez" =>
        {
          number: 11,
          shoe: 17,
          points: 17,
          rebounds: 19,
          assists: 10,
          steals: 3,
          blocks: 1,
          slam_dunks: 15
        },
        "Mason Plumlee" =>
        {
          number: 1,
          shoe: 19,
          points: 26,
          rebounds: 12,
          assists: 6,
          steals: 3,
          blocks: 8,
          slam_dunks: 5
        },
        "Jason Terry" =>
        {
          number: 31,
          shoe: 15,
          points: 19,
          rebounds: 2,
          assists: 2,
          steals: 4,
          blocks: 11,
          slam_dunks: 1
        }
      }
    },
    away:
    {
      team_name: "Charlotte Hornets",
      colors: ["Turquoise", "Purple"],
      players:
      {
        "Jeff Adrien" =>
        {
          number: 4,
          shoe: 18,
          points: 10,
          rebounds: 1,
          assists: 1,
          steals: 2,
          blocks: 7,
          slam_dunks: 2
        },
        "Bismak Biyombo" =>
        {
          number: 0,
          shoe: 16,
          points: 12,
          rebounds: 4,
          assists: 7,
          steals: 7,
          blocks: 15,
          slam_dunks: 10
        },
        "DeSagna Diop" =>
        {
          number: 2,
          shoe: 14,
          points: 24,
          rebounds: 12,
          assists: 12,
          steals: 4,
          blocks: 5,
          slam_dunks: 5
        },
        "Ben Gordon" =>
        {
          number: 8,
          shoe: 15,
          points: 33,
          rebounds: 3,
          assists: 2,
          steals: 1,
          blocks: 1,
          slam_dunks: 0
        },
        "Brendan Haywood" =>
        {
          number: 33,
          shoe: 15,
          points: 6,
          rebounds: 12,
          assists: 12,
          steals: 22,
          blocks: 5,
          slam_dunks: 12
        }
      }
    }
  }
end

# def num_points_scored(player_name)
#   game_hash.each do |team, stats|
#     stats[:players].each do |player, stat|
#       if player == player_name
#         stat.each do |type, val|
#           if type.to_s == "points"
#             return val
#           end
#         end
#       end
#     end
#   end
# end

def num_points_scored(player_name)
  game_hash.each do |team, stats|
    stats [:players].each do |player, stat|
      if player == player_name
        return stat[:points]
      end
    end
  end
end

# def shoe_size(player_name)
#   game_hash.each do |team, stats|
#     stats[:players].each do |player, stat|
#       if player == player_name
#         stat.each do |type, val|
#           if type.to_s == "shoe"
#             return val
#           end
#         end
#       end
#     end
#   end
#   end

def shoe_size(player_name)
  game_hash.each do |team, stats|
    stats [:players].each do |player, stat|
      if player == player_name
        return stat[:shoe]
      end
    end
  end
end

# def team_colors(team)
#   game_hash.each do |teams, stats|
#     if stats[:team_name] == team
#         return stats[:colors]
#     end
#   end
# end

def team_colors(team_names)
  game_hash.each do |team, stats|
   if stats[:team_name] == team_names
     return stats[:colors]
  end
end
end

# def team_names
#   name_array = []
#   game_hash.each do |teams, stats|
#     name_array << stats[:team_name]
#   end
#   name_array
# end

def team_names
  game_hash.map do |teams, stats|
    stats[:team_name]
  end
end

# def player_numbers(team)
#   team_array = []
#   game_hash.each do |teams, stats|
#     if stats[:team_name] == team
#       stats[:players].each do |player, stat|
#         team_array << stat[:number]
#       end
#     end
#   end
#   team_array
# end

def player_numbers(team)
  number = []
  game_hash.each do |teams, stats|
    if stats[:team_name] == team
      stats[:players].each do |player, stat|
        number << stat[:number]
      end
    end
  end
  number
end

# def player_stats(player_name)
#   stats_hash = {}
#   game_hash.each do |local, team_stat|
#     team_stat[:players].each do |player, stat|
#       if player == player_name
#         stat.each do |key, val|
#           stats_hash[key] = val
#         end
#       end
#     end
#   end
#   stats_hash
# end

def player_stats(name)
  player_hash = {}
  game_hash.each do |teams, stats|
    stats[:players].each do |player, stat|
      if player == name
        player_hash[stat] = stat
      end
    end
  end
  player_hash
end

def big_shoe_rebounds
  big_shoe = 1
  rebound = 0

  game_hash.each do |local, team_stat|
    team_stat[:players].each do |player, stat|
      if stat[:shoe] > big_shoe
        big_shoe = stat[:shoe]
        rebound = stat[:rebounds]
      end
    end
  end
  rebound
end

def find_player(points)
  game_hash.each do |local, team_stat|
    team_stat[:players].each do |player, stat|
      if stat[:points] == points
        puts (player)
      end
    end
  end
end

find_player(33)
